   ...
   */
   private async addArcGISFeatureLayer(layerConfig: any): Promise<void> {
    ...
    try {
        ...
        // ArcGIS servers limit how many features can be returned in one request
        // This is like a "page size" limit to prevent overwhelming the server
        const maxRecordCount = layerInfo.maxRecordCount || 1000;
      
        // Get drawing info for styling
        // Try to get it from the layer info first, otherwise fetch separately
        ...
        // If a layer has 5,000 features but the server only allows 1,000 per request,
        // we need to make 5 separate requests:
